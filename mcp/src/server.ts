...
/**
 * Builds a server bound to one Stacky user.
 *
 * The identity is a closure parameter, not module state: the HTTP transport
 * serves many people from one process, and a shared "current user" would leak
 * across concurrent requests. Every tool below passes `userId` into the scope
 * layer, which re-checks access against the database on each call.
 */
export function createStackyServer(userId: string): McpServer {
  ...
  server.registerTool(
    ...
    guard(async (args) => {
      const id = await createWorkspace(userId, args)
  ...
  server.registerTool(
    ...
    guard(async (args) => {
      const id = await createProject(userId, args)
  ...
  server.registerTool(
    ...
    guard(async ({ workspaceId, projectId, status, tag, limit }) => {
      ...
      if (workspaceId) {
        ...
        scopeIds = (await listWorkspaces(userId)).map((w) => w.id)
      }
  ...
  server.registerTool(
    ...
    guard(async ({ taskId }) => {
      const { ref, data } = await assertTask(userId, taskId)
  ...
  server.registerTool(
    ...
    guard(async (args) => {
      const id = await createTask(userId, args)
  ...
  server.registerTool(
    ...
    guard(async ({ taskId, ...rest }) => {
      const { previousStatus, title } = await updateTask(userId, taskId, rest)
  ...
  server.registerTool(
    ...
    guard(async ({ taskId, completionNote }) => {
      const { previousStatus, title } = await updateTask(userId, taskId, { status: 'done', completionNote })
  ...
  server.registerTool(
    ...
    guard(async ({ taskId, body }) => {
      const id = await addComment(userId, taskId, body)
