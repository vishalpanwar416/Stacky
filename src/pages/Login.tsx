...
export function Login() {
  const { user, loading, signInWithGoogle, signInWithGithub, authError, clearAuthError, pendingLinkEmail, completeGithubLink } = useAuth()
        ...
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left w-full max-w-xl mx-auto lg:mx-0 animate-stagger">
          ...
          <div className="mt-6 sm:mt-8 flex flex-col items-center lg:items-start w-full gap-3">
            {authError && (
              <div className="mb-4 w-full max-w-sm rounded-2xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-200">
                ...
                {pendingLinkEmail ? (
                  <button
                    type="button"
                    onClick={completeGithubLink}
                    className="mt-3 w-full rounded-xl bg-amber-500/20 px-4 py-2 text-xs font-semibold text-amber-100 transition-colors hover:bg-amber-500/30"
                  >
            ...
            <button
              ...
              className="w-full max-w-sm rounded-2xl border px-6 sm:px-8 py-3 font-medium text-sm transition-all duration-300 hover:scale-[1.01] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-(--color-accent) focus:ring-offset-2 focus:ring-offset-(--color-bg)"
