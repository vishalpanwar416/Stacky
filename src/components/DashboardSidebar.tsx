...
export function DashboardSidebar({
  ...
  onEditWorkspace: (id: string, name: string, tab?: 'general' | 'members') => void
                        ...
                        <li key={w.id} className="space-y-0.5 group relative">
                          <div className={`flex items-center rounded-xl transition-colors pr-1 ${currentWorkspace?.id === w.id
                            ...
                            <div className="relative shrink-0">
                              ...
                              {menuOpenId === w.id && (
                                <div
                                  className="absolute right-0 top-full z-50 mt-1 w-36 origin-top-right rounded-xl border theme-surface-bg theme-border shadow-lg p-1 focus:outline-none bg-[#111] dark:bg-[#111]"
                                  onClick={(e) => e.stopPropagation()}
                                >
                                  <button
                                    onClick={() => {
                                      onEditWorkspace(w.id, w.name, 'members')
                                      setMenuOpenId(null)
                                    }}
